...
import { audioInputApi, audioOutputApi } from "./windows";
...
beforeEach(() => {
	root = mkdtempSync(path.join(os.tmpdir(), "audio-source-windows-adapter-"));
	process.chdir(root);
	root = process.cwd();
	executable = path.join(root, "dev.jerez.sds.audio-source.sdPlugin/native/windows/audio-bridge.exe");
...
describe("Windows native bridge invocation", () => {
	...
	it("waits for ready, buffers changed lines across chunks, and cleans up once", async () => {
		...
		expect(watcher.kill).toHaveBeenCalledWith();
	...
	it("rejects an early exit and logs an unexpected exit after ready", async () => {
		...
		expect(earlyWatcher.stdout.listenerCount("data")).toBe(0);
		expect(earlyWatcher.stderr.listenerCount("data")).toBe(0);
		expect(earlyWatcher.listenerCount("error")).toBe(0);
		expect(earlyWatcher.listenerCount("exit")).toBe(0);
		...
		const cleanup = await readySubscription;
		readyWatcher.emit("exit", 3, null);
		expect(loggerMocks.error).toHaveBeenCalledWith(expect.stringContaining("exited unexpectedly"));
		expect(readyWatcher.stdout.listenerCount("data")).toBe(0);
		expect(readyWatcher.stderr.listenerCount("data")).toBe(0);
		expect(readyWatcher.listenerCount("error")).toBe(0);
		expect(readyWatcher.listenerCount("exit")).toBe(0);
		expect(() => {
			cleanup();
			cleanup();
		}).not.toThrow();
		expect(readyWatcher.kill).not.toHaveBeenCalled();
	});

	it("times out watcher startup, terminates the process, and detaches listeners", async () => {
		vi.useFakeTimers();
		const watcher = createWatcher();
		childProcessMocks.spawn.mockReturnValue(watcher);

		const subscription = audioOutputApi.subscribeDefaultDeviceChanges(vi.fn());
		const rejection = expect(subscription).rejects.toThrow(/timed out before ready/u);
