...
export class DeviceSwitcher {
	...
	async preview(actionId: string, ticks: number): Promise<PreviewSelection | null> {
		...
		const selectedDevice = devices.find((device) => device.id === selectedId) ?? devices[state.selectedIndex];
	...
	private async getOrCreatePendingSelection(
		...
		const defaultIndex = currentDefault ? deviceIds.findIndex((id) => id === currentDefault.id) : -1;
