...
export abstract class CycleAudioDeviceAction extends SingletonAction<ActionSettings> {
  ...
  override async onWillAppear(ev: WillAppearEvent<ActionSettings>): Promise<void> {
    try {
      ...
      const [, activeDevice] = await Promise.all([
        this.switcher.initialize(ev.action.id),
        this.getActiveDevice(),
      ]);

      await this.renderIdleAndTrack(
        ev.action.id,
        ev.action as DialAction<ActionSettings>,
        activeDevice,
      );
   ...
   */
  private async advanceBrowseState(
    actionId: string,
    selectedId: string,
  ): Promise<BrowseModel | null> {
  ...
  private async renderIdleAndTrack(
    ...
    this.lastActiveStateByActionId.set(
      actionId,
      deviceStateSignature(resolved, this.noDeviceTitle),
    );
  }
}
