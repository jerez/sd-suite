...
import {
  ...
  type DeviceRibbonCarouselModel,
} from "./components/device-ribbon-carousel";
...
export class DeviceDialRenderer {
  ...
  async renderBrowse(
    actionId: string,
    action: DialRendererAction,
    model: DeviceRibbonCarouselModel,
  ): Promise<void> {
    void actionId;
 ...
 */
function isDialActionWithLayout(
  action: unknown,
): action is { setFeedbackLayout(layout: string): Promise<void> } {
  return Boolean(
    action &&
